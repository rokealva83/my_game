...
def work_with_project(request):
    if "live" not in request.session:
        ...
        if request.POST.get('create_ship'):
            ...
            if len_turn_create_ship < 5:
                ...
                if warehouse_hull is not None and warehouse_hull.amount >= amount_ship:
                    error = 0
                    for i in range(2, 9):
                        element_ships = ElementShip.objects.filter(id_project_ship=ship_id, class_element=i).order_by(
                            'id_element_pattern')
                        work_element_id = 0
                        if element_ships:
                    ...
                    if error == 0:
                        ...
                        warehouse_hull = WarehouseElement.objects.filter(user=session_user,
                        ...
                        element_ships = ElementShip.objects.filter(id_project_ship=ship_id).order_by('class_element')
                        for element_ship in element_ships:
        ...
        if request.POST.get('delete_pattern'):
            ...
            delete_ship_pattern = ProjectShip.objects.filter(id=ship_id).delete()
