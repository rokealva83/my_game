...
def scan_veryfication(*args):
    ...
    delta = flightplan_scan.flight_time
    if new_delta > delta:
            ...
            for system in systems:
                if min_x <= system.x <= max_x and min_y <= system.y <= max_y and min_z <= system.z <= max_z:
                    ...
                    distance = round(math.sqrt(
                        (fleet_x - system_x) ** 2 + (fleet_y - system_y) ** 2 + (
                            fleet_z - system_z) ** 2), 3)
                    if distance <= delta and distance != 0:
                        line = u' %s : %s : %s - растояние: %s св. \n' % (
                            system_x, system_y, system_z, distance)
                        if distance <= delta * 0.1:
                            ...
                            for planet in planets:
                                message_planet = u'    %s - %s : %s : %s \n' % (
                                    planet.planet_num, planet.global_x, planet.global_y,
                                    planet.global_z)
                        mail_text = mail_text + line + message_planet_subject + message_planet
            ...
            for asteroid_field in asteroid_fields:
                if min_x <= asteroid_field.x <= max_x and min_y <= asteroid_field.y <= max_y and min_z <= asteroid_field.z <= max_z:
                    ...
                    distance = round(math.sqrt(
                        ...
                        3)
                    if distance <= delta:
                        line = u'%s : %s : %s  Размер:%s Растояние:%s \n' % (
                            system_x, system_y, system_z, asteroid_size, distance)
