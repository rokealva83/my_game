...
def verification_phase_of_construction(request):
    user = request
    turn_buildings = TurnBuildingBuilding.objects.filter(user=user).all()
    turn_factorys = TurnBuildingFactory.objects.filter(user=user).all()
    turns = [turn_buildings, turn_factorys]
    for turn_buildings in turns:
        time = timezone.now()
        for turn_building in turn_buildings:
            ...
            if new_delta > delta:
                ...
                if turn_building.class_id != 21:
                    ...
                    factory_pattern = FactoryPattern.objects.filter(id=turn_building.factory.id).first()
                ...
                if turn_building.class_id != 21:
                    TurnBuildingFactory.objects.filter(id=turn_building.id).delete()
                else:
                    TurnBuildingBuilding.objects.filter(id=turn_building.id).delete()
