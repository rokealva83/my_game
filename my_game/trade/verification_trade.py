            ...
            if trade_flight.status == 1:
                if trade_flight.flightplan == 1:
                    ...
                    if new_delta > delta:
                        ...
                        Fleet.objects.filter(id=fleet.id).update(x=x, y=y, z=z)
                        trade_flight = TradeFlight.objects.filter(id_fleet=fleet.id).first()
                        TradeFlight.objects.filter(id=trade_flight.id).delete()
                        trade_flight = TradeFlight.objects.filter(id_fleet=fleet.id).first()
                        TradeFlight.objects.filter(id=trade_flight.id).update(status=1)
                    ...
                    if new_delta > delta:
                        ...
                        Fleet.objects.filter(id=fleet.id).update(empty_hold=empty_hold, ship_empty_mass=new_mass)
                        ...
                        if trade_flight:
                            ...
                            if planet:
                                ...
                                planet_status = 0
                            Fleet.objects.filter(id=fleet.id).update(planet=user_city.planet, system=user_city.system,
                                                                     status=0, planet_status=planet_status)
                    ...
                    if new_delta > delta:
                        ...
                        if hold.class_shipment == 0:
                            ...
                            Warehouse.objects.filter(user=user, user_city=trade_flight.user_city,
                                                     resource_id=trade_flight.element_id).update(amount=new_amount)
                            ...
                            if warehouse:
                                new_amount = warehouse.amount + trade_flight.amount
                                WarehouseElement.objects.filter(user=user, user_city=trade_flight.user_city,
                                                                element_class=trade_flight.class_element,
                                ...
                                WarehouseElement(
                                    ...
                                    amount=trade_flight.amount).save()
                            ...
                            if warehouse:
                                new_amount = warehouse.amount + trade_flight.amount
                                WarehouseFactory.objects.filter(user=user, user_city=trade_flight.user_city,
                                                                production_class__lt=21,
                        ...
                        Hold.objects.filter(fleet=fleet).delete()
                        new_mass = fleet.ship_empty_mass - trade_flight.mass
                        empty_hold = fleet.empty_hold + trade_flight.size
                        Fleet.objects.filter(id=fleet.id).update(empty_hold=empty_hold, ship_empty_mass=new_mass)
                        trade_flight = TradeFlight.objects.filter(fleet=fleet).first()
    ...
    for teleport in teleports:
        ...
        if new_delta > delta:
            ...
            if teleport.class_element == 0:
                ...
                Warehouse.objects.filter(user=user, user_city=teleport.user_city,
                ...
                if warehouse:
                    new_amount = warehouse.amount + teleport.amount
                    WarehouseElement.objects.filter(user=user, user_city=teleport.user_city,
                                                    element_class=teleport.class_element,
                                                    element_id=teleport.element_id).update(amount=new_amount)
                else:
                    WarehouseElement(
                        user=user,
                ...
                if warehouse:
                    new_amount = warehouse.amount + teleport.amount
                    WarehouseFactory.objects.filter(user=user, user_city=teleport.user_city, production_class__lt=21,
                                                    factory=teleport.element_id).update(amount=new_amount)
                else:
            ...
            TradeTeleport.objects.filter(id=teleport_id).delete()
