...
def fleet_hold(request):
    if "live" not in request.session:
        ...
        if fleet.planet_status == 1 and ship_in_fleet is not None:
            ...
            if resource_amount[0] != 0 and shipment_id_res is not None:
                ...
                if class_shipment == 0:
                    ...
                    if warehouse.amount >= amount_shipment:
                        ...
                        Warehouse.objects.filter(user=session_user, user_city=session_user_city,
                                                 resource_id=shipment_id).update(amount=new_amount)
                    else:
                        error = 1
        ...
        basic_resources = BasicResource.objects.filter().all()
        ...
        output = {'user': session_user, 'warehouse': session_user_city.warehouse, 'basic_resources': basic_resources,
                  ...
                  'add_ships': add_ships, 'fleet_id': fleet_id, 'ship_fleets': ship_fleets,
                  'fuel_patterns': fuel_patterns, 'shell_patterns': shell_patterns, 'ships': ships, 'command': command,
