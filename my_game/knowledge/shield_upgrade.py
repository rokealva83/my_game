...
def shield_upgrade(request):
    ...
    user_shield = ShieldPattern.objects.filter(user=user, basic_pattern=shield_scient).last()
    if user_shield is None:
        ...
        studied_shield = ShieldPattern.objects.filter(user=user, basic_pattern=shield_scient, bought_template=0)
